    ...
    pub fn instant_update(&mut self, app: &mut ApplicationContext) {
        ...
        *self.refresh.end_date.lock().unwrap() = end;
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            ...
            thread::spawn(move || {
                let res = get_current_jobs_for_period(start, end, &ssh_host, "./data/data.json", &*version);
                // Discard if a newer request has already been dispatched.
                if *request_gen.lock().unwrap() != gen {
                    return;
                }
                if res {
                    let jobs = get_jobs_from_json("./data/data.json", &*version);
                    let resources = get_resources_from_json("./data/data.json", &*version);
